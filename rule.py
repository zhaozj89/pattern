...
root_node = {'id': None, 'form': '', 'lemma': '_', 'upos': 'SPACE', 'xpos': '_SP', 'feats': None, 'head': 0, 'deprel': 'ROOT', 'deps': None, 'misc': None}
def process_node(node, node_list):
    if node is None:
        node = root_node
        ids = []
        for i in range(len(node_list)):
            ids.append(node_list[i]['id'])
        for i in range(len(node_list)):
            if i in ids:
                pass
            else:
...
with open('/home/amax/zzhaoao/BookComprehension/pipeline/questions/pattern/preps.txt', 'r') as f:
...
class Question:
    def __init__(self, question):
        ...
        self.root = process_node(self._get_node('ROOT'), self.question)
...
class AnswerSpan:
    def __init__(self, answer):
        ...
        # self.root = self._get_rel('ROOT')
        self.root = process_node(self._get_rel('ROOT'), self.answer)
